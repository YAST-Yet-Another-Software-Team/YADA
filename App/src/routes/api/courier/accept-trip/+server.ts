...
export const POST: RequestHandler = apiRoute(
	...
	async ({ request }, user) => {
		...
		const claimed = await applyTripChange(
			tripId,
			[eq(deliveryRequests.status, 'requested'), isNull(deliveryRequests.assignedCourierId)],
			{ assignedCourierId: user.id, status: 'accepted', acceptedAt: new Date() }
		);
		...
		if (!claimed) {
			...
			return (await tripExists(tripId))
				? apiError(409, 'conflict', 'Another rider has already taken this delivery.')
